...
int
poll (struct pollfd *pfd, nfds_t nfd, int ftpd_timeout)
{
  ...
  /* convert ftpd_timeout number into a timeval structure */
  if (ftpd_timeout == 0)
    {
      ptv = &tv;
      ptv->tv_sec = 0;
  ...
  else if (ftpd_timeout > 0)
    {
      ptv = &tv;
      ptv->tv_sec = ftpd_timeout / 1000;
      ptv->tv_usec = (ftpd_timeout % 1000) * 1000;
    }
  else if (ftpd_timeout == INFTIM)
    /* wait forever */
    ptv = NULL;
  else
    {
...
restart:
  ...
  if (select (0, &rfds, &wfds, &xfds, &tv0) > 0)
    ...
    {
      poll_again = TRUE;
      if (ftpd_timeout == INFTIM)
        wait_timeout = INFINITE;
      else
        wait_timeout = ftpd_timeout;
    }
