...
#undef timeval
{
  ...
  if (!ftpd_timeout)
    ...
    {
      wait_timeout = ftpd_timeout->tv_sec * 1000 + ftpd_timeout->tv_usec / 1000;
  ...
  if (nhandles > 1)
    {
      ...
      if (ftpd_timeout && wait_timeout == 0 && rc == 0)
        ftpd_timeout->tv_sec = ftpd_timeout->tv_usec = 0;
    }
            ...
            struct timeval *ftpd_timeout)
{
  ...
  return select (nfds, rfds, wfds, xfds, ftpd_timeout);
}
